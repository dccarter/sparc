...
struct kore_timer
*kore_timer_new(void (*cb)(void *, u_int64_t), u_int64_t interval,
         void *arg, int flags)
{
...
void
kore_timer_set(struct kore_timer *timer)
{
    struct kore_timer *t;
    timer->nextrun = kore_time_ms() + timer->interval;

    if (!(timer->flags & KORE_TIMER_INPLACE)) {
        timer->flags |= KORE_TIMER_INPLACE;
    ...
    void *arg, int flags)
{
    ...
    timer = kore_timer_new(cb, interval, arg, flags);

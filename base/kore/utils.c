...
void
kore_logs(int prio, const char *buf)
{
    int  color = CDEFAULT;
    char tmp[32];
    if (foreground) {
...
#endif
        if (foreground)
            kore_color_printf(color, "[%s]: %s\n", tmp, buf);
        else
            syslog(prio, "[%s]: %s", tmp, buf);
    } else {
        if (foreground)
            kore_color_printf(color, "[parent]: %s\n", buf);
        else
...
void
kore_color_vprintf(int color, const char* fmt, va_list args)
{
    if (color > 0 && color <= CCYAN)
...
void
kore_color_printf(int color, const char  *fmt, ...) {
    va_list    args;
    va_start(args, fmt);
    kore_color_vprintf(color, fmt, args);
    va_end(args);
}
